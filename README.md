...
这是一个供小团队使用的 Windows/macOS 本地排课工具。每位使用者可以从公开 GitHub 仓库下载同一套程序，在自己的电脑上运行，并使用自己的本地数据库；配置腾讯会议企业 API 后，可以创建、查询和同步对应企业中的腾讯会议。
...
同事第一次克隆后，以后不需要删除项目或重新克隆，也不需要重新配置腾讯会议 API。每次更新只需：

1. 停止服务。
2. 拉取 GitHub 最新代码。
3. 重新启动服务。

本地 API 环境变量、Edge 登录状态和 `schedule_data.db` 不会被 `git pull` 覆盖。
...
- 团队成员离职或不再需要调用企业 API 时，应及时轮换或撤销其本地使用的 API 凭据。
