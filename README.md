...
这是一个供小团队使用的 Windows/macOS 本地排课工具。每位使用者可以从公开 GitHub 仓库下载同一套程序，在自己的电脑上运行，并使用自己的本地数据库。企业账号可以通过腾讯会议企业 API 同步，个人账号可以通过各自的 MCP Token 同步会议和查询录制回放。

## 项目背景

这个项目源于小团队日常安排腾讯会议时遇到的实际问题：团队需要在多个会议账号和教室之间反复确认空闲时间，人工查看后台、切换账号和记录排课不仅耗时，也容易出现时间冲突、漏记会议、重复创建以及登录错账号等情况。

项目早期采用一台电脑启动服务、其他人在同一局域网中访问的方式。这种方式要求主机始终开机，使用者还必须处在同一个网络中，不适合异地协作和长期使用。因此，项目后来调整为“公开代码、各自本地运行”的模式：

- 每个人都可以从 GitHub 获取和更新程序，不再依赖某一台电脑持续提供服务。
- 每台电脑使用独立的本地数据库，个人导入的数据和本地备注不会被上传到 GitHub。
- 团队成员可以配置同一企业的腾讯会议 API，同步企业中已经创建的会议。
- 个人腾讯会议账号可以单独配置 MCP Token，同步已结束、进行中和即将开始的会议，并查询对应录制回放。
- API 凭据只保存在使用者自己的电脑中，不随代码分发，也不会提交到仓库。

本项目不是在线 SaaS 服务，也不提供公共云端数据库。它的目标是降低腾讯会议排课、账号切换、时间冲突检查和会议记录查询的操作成本，同时保留本地部署简单、数据彼此独立的特点。

## 项目现状

项目目前已经成熟并进入稳定使用阶段，经过实际工作场景的持续使用和完善，获得了同事的一致好评。它明显减少了人工查询空闲账号、切换会议账号、核对时间冲突和整理会议记录所需的重复操作，大大提高了团队的排课与会议管理效率。

### 使用界面示例

![腾讯会议排课调度工具使用界面](docs/images/scheduling-overview.png)
...
- 企业账号通过统一的企业 API 同步；个人账号通过“账号管理”中保存的 MCP Token 同步，两类结果会合并到本地排课记录。
...
3. 搜索并选择 `gsj1225/tencent-meeting-scheduler`；也可以通过仓库页面的 `Code` → `Open with GitHub Desktop` 打开。
...
这是公开仓库，正常克隆不需要仓库访问权限。如果使用 GitHub Desktop，可以登录自己的 GitHub 账号，也可以通过仓库页面直接打开。
...
工具支持企业账号和个人账号两种同步方式，可以只配置其中一种，也可以同时使用。

### 企业账号：配置腾讯会议 API

每台电脑都要单独配置一次。企业 API 凭据保存在本机环境变量中，不会保存在 GitHub 项目或本地数据库中。
...
个人腾讯会议账号不使用上面的企业 API 凭据，需要为每个账号单独保存已经取得的 MCP Token：

1. 启动工具，打开“账号管理”。
2. 添加或找到对应的个人腾讯会议账号。
3. 在该账号的 `MCP Token` 输入框中粘贴 Token。
4. 点击“保存”，账号旁边出现 `MCP` 标识即表示已经配置。
5. 回到首页点击“同步腾讯会议”，工具会同步该账号最近的已结束会议，以及进行中和即将开始的会议。

MCP Token 会保存在当前电脑的 `schedule_data.db` 中，只用于本机向腾讯会议 MCP 接口发起请求。它不会自动上传到 GitHub，也不会在不同电脑之间同步。换电脑或重新建立本地数据库后，需要重新填写。

MCP Token 属于敏感凭据，不要将它写进代码、README、Issue、聊天记录或公开截图，也不要把包含 Token 的数据库发送给其他人。
...
- 已配置企业 API 时，工具会同步企业账号会议。
- 账号管理中配置了 MCP Token 时，工具会同时同步对应个人账号会议。
- 同一个会议号只保留一份记录，企业 API 和 MCP 返回的结果会自动合并去重。
...
1. 在“排课”页面填写日期和起止时间，也可以粘贴日期时间消息让工具自动识别。
2. 查找空闲账号并选择一个账号。
...
1. 在页面下方“查询回放 / 取消会议”区域输入会议号。
2. 点击“查询回放”。
3. 工具会同时查询企业 API 和已经配置 MCP Token 的个人账号。
4. 如果同一会议存在多个录制文件，工具会优先返回时长最长的一条录制。
5. 返回内容保持固定的“录制、日期、录制文件”格式，可以直接复制使用。

个人账号的录制查询会并发执行并短暂缓存结果，重复查询通常会更快。如果已经找到录制但没有链接，请先在腾讯会议中确认该录制已经开启共享。

## 九、更新到最新版本

同事第一次克隆后，以后不需要删除项目或重新克隆，也不需要重新配置腾讯会议企业 API 或个人账号 MCP Token。每次更新只需：

1. 停止服务。
2. 拉取 GitHub 最新代码。
3. 重新启动服务。

本地 API 环境变量、Edge 登录状态、MCP Token 和 `schedule_data.db` 不会被 `git pull` 覆盖。
...
仓库是公开的，不需要管理员邀请。请检查仓库地址是否正确、网络是否能访问 GitHub，以及 GitHub Desktop 是否已完成初始化。仓库地址为：

```text
https://github.com/gsj1225/tencent-meeting-scheduler
```
...
1. “账号管理”中对应账号是否显示 `MCP` 标识。
2. MCP Token 是否完整、是否已经失效。
3. Token 是否误填到了其他账号。
4. 网络是否可以访问腾讯会议 MCP 接口。

### 输入会议号后查不到个人账号回放

依次确认会议号是否正确、对应个人账号是否已经配置 MCP Token、会议是否确实产生了云录制，以及录制是否已经开启共享。刚结束的会议可能需要等待腾讯会议完成录制处理后才能查询。
...
- 这是公开仓库，提交前必须确认改动中不包含真实业务数据或内部敏感信息。
...
- MCP Token 与 SecretKey 同样属于敏感凭据，不要提交到 GitHub 或放进公开截图。
- `schedule_data.db` 可能包含账号的 MCP Token；备份和转交数据库前必须确认接收方可信。
- 只有需要共同维护代码的人才应获得仓库写入权限；普通使用者直接 Clone 或下载即可。
- 团队成员离职或不再需要调用腾讯会议接口时，应及时轮换企业 API 凭据并撤销对应 MCP Token。
